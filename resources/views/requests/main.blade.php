 ...
 <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <button class="bg-white p-4 rounded-xl border border-gray-200 text-left transition hover:scale-[1.02] shadow-sm ring-2 ring-blue-500">
            <div class="flex justify-between items-start">
              <span class="text-3xl font-bold text-gray-800">{{ count($requests) }}</span>
          ...
          <button class="bg-white p-4 rounded-xl border border-gray-200 text-left transition hover:scale-[1.02] shadow-sm">
            <div class="flex justify-between items-start">
              <span class="text-3xl font-bold text-gray-800">{{ $requests->where('status', 'Pending')->count() }}</span>
          ...
          <button class="bg-white p-4 rounded-xl border border-gray-200 text-left transition hover:scale-[1.02] shadow-sm">
            <div class="flex justify-between items-start">
              <span class="text-3xl font-bold text-gray-800">{{ $requests->where('status', 'approved')->count() }}</span>
          ...
          <button class="bg-white p-4 rounded-xl border border-gray-200 text-left transition hover:scale-[1.02] shadow-sm">
            <div class="flex justify-between items-start">
              <span class="text-3xl font-bold text-gray-800">{{ $requests->where('status', 'revision')->count() }}</span>
          ...
          <button class="bg-white p-4 rounded-xl border border-gray-200 text-left transition hover:scale-[1.02]">
            <div class="flex justify-between items-start">
              <span class="text-3xl font-bold text-gray-800">{{ $requests->where('status', 'rejected')->count() }}</span>
  ...
  <script>
    ...
    function toggleDropdown() {
      ...
      if(arrow) arrow.style.transform = dropdown.classList.contains('hidden') ? 'rotate(-90deg)' : 'rotate(0deg)';
    }

    function openNewRequestModal() {
      document.getElementById('newRequestModal').classList.remove('hidden');
      if (itemsWrapper && itemsWrapper.children.length === 0) addItemRow();
    }
    ...
    function handleFileSelected(input) {
      if(input.files && input.files[0]) {
        ...
        const badge = document.getElementById('file-badge-preview');
    ...
    function clearSelectedFile(event) {
      ...
      const input = document.getElementById('supporting_doc');
      input.value = '';
      document.getElementById('file-badge-preview').classList.add('hidden');
    ...
    function addItemRow() {
      ...
      div.id = rowId;
      div.className = "item-row bg-gray-50 p-4 rounded-xl border border-gray-200 relative mb-4";
      div.innerHTML = `
        <button type="button" onclick="removeItem('${rowId}')" class="absolute top-2 right-2 text-gray-400 hover:text-red-500"><i data-lucide="x" class="w-4 h-4"></i></button>
        <div class="grid grid-cols-2 gap-3">
            <input type="text" name="items[name][]" placeholder="Product Name *" required class="col-span-2 px-3 py-2 border rounded text-sm bg-white">
            <select name="items[category][]" class="px-3 py-2 border rounded text-sm bg-white" required>
                <option value="">Category</option>
                <option value="CPU">CPU</option>
                <option value="GPU">GPU</option>
                <option value="RAM">RAM</option>
            </select>
            <input type="text" name="items[brand][]" placeholder="Brand" class="px-3 py-2 border rounded text-sm bg-white">
            <input type="number" name="items[qty][]" placeholder="Qty" required class="qty-input px-3 py-2 border rounded text-sm bg-white" oninput="calculateTotal()">
            <input type="number" name="items[price][]" placeholder="Est. Price" required class="price-input px-3 py-2 border rounded text-sm bg-white" oninput="calculateTotal()">
        </div>`;
    ...
    function removeItem(id) {
      if (itemsWrapper.children.length > 1) {
        ...
        alert("You must have at least one item row in the document configuration.");
      }
    }
    ...
    function calculateTotal() {
      ...
      if(grandTotalEl) grandTotalEl.innerText = '₱' + total.toLocaleString(undefined, {minimumFractionDigits: 2});
    }

    function selectPriority(btn, level) {
      document.querySelectorAll('#priority-options button').forEach(b => {
          b.classList.remove('ring-2', 'ring-blue-500');
      });
      btn.classList.add('ring-2', 'ring-blue-500');
      const hiddenPriority = document.getElementById('hidden-priority');
      if(hiddenPriority) hiddenPriority.value = level;
    }

    function handleContinue() {
      if (currentStep === 1) {
        if (!document.getElementById('emp_name').value || !document.getElementById('emp_dept').value) {
          alert("Please fill up required Employee details before proceeding.");
          return;
        }
        ...
        document.getElementById('step-1-indicator').className = "w-8 h-8 rounded-full bg-blue-900 border border-blue-500 text-blue-200 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-2-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-label').innerText = "Step 2 of 4 — Line Items Listing Setup";
        currentStep = 2;
      } else if (currentStep === 2) {
        let valid = true;
        document.querySelectorAll('.item-row input[required], .item-row select[required]').forEach(el => {
            if(!el.value) valid = false;
        });
        if(!valid) { alert("Please complete all required item rows before moving forward."); return; }
        
        document.getElementById('step-2-form').classList.add('hidden');
        document.getElementById('step-3-form').classList.remove('hidden');
        document.getElementById('step-2-indicator').className = "w-8 h-8 rounded-full bg-blue-900 border border-blue-500 text-blue-200 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-3-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-label').innerText = "Step 3 of 4 — Logistics Vendor Routing";
        currentStep = 3;
      } else if (currentStep === 3) {
        if (!document.getElementById('supplier').value || !document.getElementById('estimated_delivery').value) {
          alert("Logistics parameter missing! Supplier and delivery dates are explicitly required.");
          return;
        }
        document.getElementById('step-3-form').classList.add('hidden');
        document.getElementById('step-4-form').classList.remove('hidden');
        document.getElementById('step-3-indicator').className = "w-8 h-8 rounded-full bg-blue-900 border border-blue-500 text-blue-200 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-4-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('submit-btn').innerText = "Submit Form Request";
        document.getElementById('step-label').innerText = "Step 4 of 4 — Summary Justification Validation";
        currentStep = 4;
      } else if (currentStep === 4) {
        if(!document.getElementById('justification').value.trim()){
            alert("Submission error. Please state your formal rationale parameter within the justification zone.");
          return;
        }
        document.querySelector('#newRequestModal form').submit();
      }
    }
    ...
    function goBack() {
      if (currentStep === 2) {
        ...
        document.getElementById('step-2-indicator').className = "w-8 h-8 rounded-full bg-blue-800 text-blue-400 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-1-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        currentStep = 1;
      } else if (currentStep === 3) {
        document.getElementById('step-3-form').classList.add('hidden');
        document.getElementById('step-2-form').classList.remove('hidden');
        document.getElementById('step-3-indicator').className = "w-8 h-8 rounded-full bg-blue-800 text-blue-400 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-2-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        currentStep = 2;
      } else if (currentStep === 4) {
        document.getElementById('step-4-form').classList.add('hidden');
        document.getElementById('step-3-form').classList.remove('hidden');
        document.getElementById('submit-btn').innerText = "Continue";
        document.getElementById('step-4-indicator').className = "w-8 h-8 rounded-full bg-blue-800 text-blue-400 flex items-center justify-center font-bold text-sm mx-2";
        document.getElementById('step-3-indicator').className = "w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm mx-2";
        currentStep = 3;
      }
    }

    function handleMultipleFilesSelected(input) {
      const container = document.getElementById('file-list-preview');
      container.innerHTML = '';
      if(input.files) {
        Array.from(input.files).forEach(file => {
          const badge = document.createElement('div');
          badge.className = "bg-blue-50 text-blue-700 p-1.5 rounded border border-blue-200 text-[11px] font-bold";
          badge.innerHTML = `<i class="fa-solid fa-file-lines mr-1"></i> ${file.name}`;
...
fetch(`/requests/${activeRequestId}/update-status`, {
    ...
    body: JSON.stringify({ 
      ...
      // Sa iyong main.blade.php, palitan ang .catch part ng ganito:
      .catch(err => {
          console.error("DEBUG ERROR:", err); 
          alert("Error details: " + err.message); // Mas detalyadong alert
      });
    }
