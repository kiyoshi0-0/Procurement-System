...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="bg-white rounded-2xl shadow border border-slate-200 p-6">
        <h2 class="font-bold text-lg mb-5">Delivery Status</h2>
        <div class="space-y-4">
            <div class="flex justify-between"><span>Received</span><span class="px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold">Completed</span></div>
            <div class="flex justify-between"><span>Inspection</span><span class="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-bold">Ongoing</span></div>
            <div class="flex justify-between"><span>Issue Investigation</span><span class="px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold">Active</span></div>
            <div class="flex justify-between"><span>Finance Hold</span><span class="px-3 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold">Waiting</span></div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow border border-slate-200 p-6">
        ...
        <div class="space-y-3">
            <div class="bg-red-100 rounded-xl p-3">🚚 Delayed shipment detected.</div>
            <div class="bg-yellow-100 rounded-xl p-3">📦 Quantity mismatch reported.</div>
            <div class="bg-blue-100 rounded-xl p-3">🔍 Waiting for warehouse inspection.</div>
            <div class="bg-green-100 rounded-xl p-3">✔ Ready for resolution.</div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-300 shadow overflow-hidden">
    <div class="p-5 border-b border-slate-200 bg-white">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <h3 class="text-lg font-semibold text-slate-800">Delivery Issue Records</h3>

            <div class="flex flex-wrap items-center gap-2">
                <input type="text" id="searchIssue" placeholder="Search issue..." class="w-64 px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none">
                ...
                <!-- Tamang Add Button na may onclick -->
                <button type="button" onclick="openAddModal()" class="px-4 py-2 bg-[#00B074] text-white rounded-lg text-sm font-semibold cursor-pointer">
                    Add Issue
                </button>
            </div>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
           ...
           <td class="px-5 py-4 font-semibold">ISS-{{ str_pad($loop->iteration, 3, '0', STR_PAD_LEFT) }}</td>
                <td class="px-5 py-4">{{ $issue->receipt_number ?? '-' }}</td>
                <td class="px-5 py-4">{{ $issue->supplier ?? '-' }}</td>
                <td class="px-5 py-4">{{ $issue->item_name ?? '-' }}</td>
                <td class="px-5 py-4">{{ $issue->issue_type ?? '-' }}</td>
                <td class="px-5 py-4">
                    @if(($issue->priority ?? '') == 'High')
                        <span class="px-3 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold">High</span>
                    @elseif(($issue->priority ?? '') == 'Medium')
                        <span class="px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold">Medium</span>
                    @else
                        <span class="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-bold">Low</span>
                    @endif
                </td>
                <td class="px-5 py-4">
                    @if(($issue->status ?? '') == 'Resolved')
                        <span class="px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold">Resolved</span>
                    @elseif(($issue->status ?? '') == 'Investigating')
                        <span class="px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold">Investigating</span>
                    @else
                        <span class="px-3 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold">Pending</span>
                    @endif
                </td>
            ...
            @empty
            <tr>
                <td colspan="9" class="text-center py-10 text-slate-400">No delivery issues found.</td>
            </tr>
            @endforelse
            </tbody>
        </table>
...
View Modal
=========================== -->
<div id="viewIssueModal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl w-[650px] max-h-[90vh] overflow-y-auto shadow-xl">
        <div class="flex items-center justify-between border-b p-5">
            <h2 class="text-lg font-bold">Delivery Issue Details</h2>
            <button type="button" onclick="closeViewModal()" class="text-slate-500 hover:text-red-600 text-xl">✕</button>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-2 gap-5">
                <div>
                    <label class="text-xs text-slate-500">Receipt Number</label>
                    <div id="modalReceipt" class="mt-1 border rounded-lg p-3 bg-slate-50">-</div>
                </div>
                <div>
                    <label class="text-xs text-slate-500">Supplier</label>
                    <div id="modalSupplier" class="mt-1 border rounded-lg p-3 bg-slate-50">-</div>
                </div>
                <div>
                    <label class="text-xs text-slate-500">Item</label>
                    <div id="modalItem" class="mt-1 border rounded-lg p-3 bg-slate-50">-</div>
                </div>
                <div>
                    <label class="text-xs text-slate-500">Issue Type</label>
                    <div id="modalType" class="mt-1 border rounded-lg p-3 bg-slate-50">-</div>
                </div>
            </div>
        </div>
    </div>
...
Edit Modal
=========================== -->
<div id="editIssueModal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl w-[650px] shadow-xl">
        <div class="flex justify-between items-center border-b p-5">
            <h2 class="text-lg font-bold">Edit Delivery Issue</h2>
        ...
        <form id="editForm" method="POST">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Supplier</label>
                        <input type="text" name="supplier" id="editSupplier" class="border rounded-lg p-3 w-full">
                    </div>
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Item</label>
                        <input type="text" name="item_name" id="editItem" class="border rounded-lg p-3 w-full">
                    </div>
                    <div>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-4 py-2 rounded bg-[#00B074] text-white">Save Changes</button>
                </div>
            </div>
        </form>
    </div>
...
Add Issue Modal
=========================== -->
<div id="addIssueModal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl w-[650px] shadow-xl">
        <div class="flex justify-between items-center border-b p-5">
            <h2 class="text-lg font-bold">Add Delivery Issue</h2>
            <button type="button" onclick="closeAddModal()">✕</button>
        </div>

        <form action="{{ route('delivery-issues.store') }}" method="POST">
            @csrf
            <div class="p-6">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Receipt Number</label>
                        <input type="text" name="receipt_number" required class="border rounded-lg p-3 w-full" placeholder="e.g. GR-0001">
                    </div>
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Supplier</label>
                        <input type="text" name="supplier" required class="border rounded-lg p-3 w-full" placeholder="Supplier Name">
                    </div>
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Item Name</label>
                        <input type="text" name="item_name" required class="border rounded-lg p-3 w-full" placeholder="Item Name">
                    </div>
                    <div>
                        <label class="text-xs text-slate-500 mb-1 block">Issue Type</label>
                        <select name="issue_type" class="border rounded-lg p-3 w-full">
                    ...
                    <div>
                        ...
                        <select name="priority" class="border rounded-lg p-3 w-full">
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" onclick="closeAddModal()" class="px-4 py-2 rounded bg-slate-200">Cancel</button>
                    <button type="submit" class="px-4 py-2 rounded bg-[#00B074] text-white">Save Issue</button>
                </div>
            </div>
        </form>
    </div>
...
// View Modal
function openViewModal(receipt, supplier, item, type) {
    document.getElementById("modalReceipt").innerText = receipt;
    document.getElementById("modalSupplier").innerText = supplier;
    document.getElementById("modalItem").innerText = item;
    document.getElementById("modalType").innerText = type;
    document.getElementById("viewIssueModal").classList.remove("hidden");
}
...
// Edit Modal
function openEditModal(id, supplier, item, type, status) {
